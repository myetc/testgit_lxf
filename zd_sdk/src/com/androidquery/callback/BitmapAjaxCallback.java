 ...
 */
@SuppressLint("NewApi")
public class BitmapAjaxCallback extends
		AbstractAjaxCallback<Bitmap, BitmapAjaxCallback> {
	...
	private static Bitmap bitmap_none = Bitmap.createBitmap(1,1, Bitmap.Config.ARGB_8888);
	...
	/**
	 * Set the preset bitmap. This bitmap will be shown immediately until the
	 * ajax callback returns the final image from the url.
	 * 
	 * @param preset
	 *            the preset
	 * @return self
	 ...
	 * 
	 * Value of 1 implies show top end of the image, 0 implies at the center, -1
	 * implies show at the bottom.
	 * 
	 * A special value AQuery.ANCHOR_DYNAMIC will adjust the anchor base. This
	 * setting will add up from 0 to 0.5 bias and it's suitable for portraits
	 * and common photos.
	 * 
	 ...
	 * 
	 * Note that the current implementation transform the image to a new one and
	 * will use more transient resources.
	 * 
	 * @param radius
	 * @return self
	 */
	...
	private static Bitmap decode(String path, byte[] data,
			BitmapFactory.Options options, boolean rotate) {
	...
	private static Bitmap rotate(String path, Bitmap bm) {
		...
		try {
			...
			ori = ei.getAttributeInt(ExifInterface.TAG_ORIENTATION,
	 ...
	 *            corner radius
	 * @param rotate
	 *            auto rotate with exif data
	 * @return the resized image
	 */
	public static Bitmap getResizedImage(String path, byte[] data, int target,
			boolean width, int round, boolean rotate) {
		...
		}else if(round == AQuery.ROUND_CIRCLE){
	...
	protected File accessFile(File cacheDir, String url) {
		//System.out.println("锟斤拷锟斤拷锟侥硷拷锟斤拷锟斤拷:"+cacheDir);
		if (imageFile != null && imageFile.exists()) {
			return imageFile;
		}

		return super.accessFile(changeFilePath(cacheDir), url);
	}

	@Override
	protected Bitmap fileGet(String url, File file, AjaxStatus status) {
		Bitmap bit = bmGet(file.getAbsolutePath(), null);
		//System.out.println("取锟斤拷锟斤拷锟斤拷锟侥硷拷锟斤拷锟斤拷:"+bit);
		return bit;
	}
	...
	public Bitmap transform(String url, byte[] data, AjaxStatus status) {
		...
		if (bm == null) {
			...
			// invalidating the file if it's not an image, could be caused by
			// proxy returning 200 with html data
	...
	private Bitmap getFallback() {
		...
		if (view != null) {
			...
			if (bm == null) {
				bm = BitmapFactory
						.decodeResource(view.getResources(), fallback);
	...
	private static Bitmap empty = Bitmap.createBitmap(1, 1,
	...
	private static Bitmap dummy = Bitmap.createBitmap(1, 1,
			Bitmap.Config.ALPHA_8);
	...
	private void checkCb(BitmapAjaxCallback cb, String url, ImageView v,
	...
	/**
	 * Sets the icon cache size in count. Icons are images less than 50x50
	 * pixels.
	 * 
	 * @param limit
	 *            the new icon cache limit
	 */
	...
	/**
	 * Sets the file cache write policy. If set to true, images load from
	 * network will be served quicker before caching to disk, this however
	 * increase the chance of out of memory due to memory allocation.
	 * 
	 * Default is false.
	 * 
	 * @param limit
	...
	/**
	 * Sets the pixel limit per image. Image larger than limit will not be
	 * memcached.
	 * 
	 * @param pixels
	 *            the new pixel limit
	...
	/**
	 * Sets the pixel criteria for small images. Small images are cached in a
	 * separate cache.
	 * 
	 * Default is 50x50 (2500 pixels)
	 * 
	 * @param pixels
	 *            the small image pixel criteria
	 */
	...
	/**
	 * Sets the max pixel limit for the entire memcache. LRU images will be
	 * expunged if max pixels limit is reached.
	 * 
	 * @param pixels
	 *            the new max pixel limit
	 */
	...
	private static Map<String, Bitmap> getBCache() {
		if (bigCache == null) {
			bigCache = Collections.synchronizedMap(new BitmapCache(BIG_MAX,
					BIG_PIXELS, BIG_TPIXELS));
		}
		return bigCache;
	}
	...
	private static Map<String, Bitmap> getSCache() {
		if (smallCache == null) {
			smallCache = Collections.synchronizedMap(new BitmapCache(SMALL_MAX,
					SMALL_PIXELS, 250000));
		}
		return smallCache;
	}
	...
	private static Map<String, Bitmap> getICache() {
		if (invalidCache == null) {
			invalidCache = Collections.synchronizedMap(new BitmapCache(100,
					BIG_PIXELS, 250000));
		}
		return invalidCache;
	}
	...
	public static boolean isMemoryCached(String url) {
		return getBCache().containsKey(url) || getSCache().containsKey(url)
				|| getICache().containsKey(url);
	}
	 ...
	 *            the url
	 * @param targetWidth
	 *            the target width, 0 for non downsampling
	...
	public static void memPut(String url, int targetWidth, int round,
			Bitmap bm, boolean invalid) {
		...
		if (targetWidth > 0 || round > 0) {
			...
			// to indicate that the variant of that url is cached by puting and
			// empty value
	...
	protected File getCacheFile() {
		...
		return AQUtility.getCacheFile(changeFilePath(cacheDir), getCacheUrl());
		
	}
	
	public static File changeFilePath(File cacheDir){
		//System.out.println("锟斤拷锟斤拷锟絞etCacheFile");
		String absPath = cacheDir.getAbsolutePath();
		//System.out.println("cacheDir.getAbsolutePath():"+absPath);
		if(absPath!=null&&absPath.contains("images")){
			return cacheDir;
		}else{
			File file = new File(absPath+"/images/");
			//System.out.println("cacheDir.getAbsolutePath()2:"+file.getAbsolutePath());
			if(!file.exists()){
				file.mkdirs();
			};
			return file;
...
//				setBitmap(url, v, null, true);
	...
	private void setBitmap(String url, ImageView iv, Bitmap bm, boolean isPreset) {
		...
		if (status != null) {
			setBmAnimate(iv, bm, preset, fallback, animation, ratio, anchor,
					status.getSource());
		}
	...
	private static Bitmap getMaxBitmap(Bitmap bm) {
		...
		if (dw > maxValue || dh > maxValue) {
			float r = (float) dh / (float) dw;
			if (r > 1.0) {
				ww = (int) (hh / r);
			} else {
				hh = (int) (ww * r);
			...
			float scaleWidth = ((float) ww) / dw;
			float scaleHeight = ((float) hh) / dh;
			Matrix matrix = new Matrix();
			matrix.postScale(scaleWidth, scaleHeight);
			Bitmap resizedBitmap = Bitmap.createBitmap(bm, 0, 0, dw, dh,
					matrix, true);
			resizedBitmap = getMaxBitmap(resizedBitmap);
	...
	private static void setBmAnimate(ImageView iv, Bitmap bm, Bitmap preset,
			int fallback, int animation, float ratio, float anchor, int source) {
	...
	private static boolean fadeIn(int animation, int source) {
		...
		case AQuery.FADE_IN_FILE:
			if (source == AjaxStatus.FILE)
				return true;
		case AQuery.FADE_IN_NETWORK:
			if (source == AjaxStatus.NETWORK)
				return true;
		default:
			return false;
		}
	...
	public static void async(Activity act, Context context, ImageView iv,
			String url, Object progress, AccountHandle ah,
			ImageOptions options, HttpHost proxy, String networkUrl) {

		async(act, context, iv, url, options.memCache, options.fileCache,
				options.targetWidth, options.fallback, options.preset,
				options.animation, options.ratio, options.anchor, progress, ah,
				options.policy, options.round, proxy, networkUrl,options.loadAnim,options.loadResid,options.interpolator);
	 ...
	 * 
	 * @param iv
	 * @param loadAnim  AQuery.LOADANIM_ROTATE is rotate . else res id
	 */
	private static void addLoadAnim(ImageView iv,int loadAnim,int loadResid,Interpolator interpolator){
		if(iv==null)return;
		Animation anim = null;
		if (loadAnim==AQuery.LOADANIM_ROTATE) {
			...
			anim = AnimationUtils.loadAnimation(iv.getContext(), loadAnim);
		}else{
			anim = null;
		}
...
//			setAnimation
		...
		if(loadResid == AQuery.NONE){
			iv.setImageBitmap(bitmap_none);
		}else {
			iv.setImageResource(loadResid);
		}
		}
	/**
	 * AQuery internal use only. Please uses AQuery image() methods instead.
	...
	public static void async(Activity act, Context context, ImageView iv,
			String url, boolean memCache, boolean fileCache, int targetWidth,
			int fallbackId, Bitmap preset, int animation, float ratio,
			float anchor, Object progress, AccountHandle ah, int policy,
			int round, HttpHost proxy, String networkUrl,int loadAnim,int loadResid,Interpolator interpolator) {
		
		
		addLoadAnim(iv,loadAnim,loadResid,interpolator);
		...
		if (bm != null) {
			...
			cb.url(url).imageView(iv).memCache(memCache).fileCache(fileCache)
					...
					.round(round).networkUrl(networkUrl);
			if (proxy != null) {
	...
	public void async(Context context) {
		...
		if (url == null) {
			...
			//no url , set fallback to imageview 
//			say:lixifeng
