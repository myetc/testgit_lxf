...
public abstract class AbstractAQuery<T extends AbstractAQuery<T>> implements Constants {
	...
	public T image(String url, boolean memCache, boolean fileCache,int loadAnim,int loadResid,Interpolator interpolator){		
		return image(url, memCache, fileCache, 0, 0,loadAnim,loadResid,interpolator);
	}
	...
	public T image(String url, boolean memCache, boolean fileCache, int targetWidth, int fallbackId,int loadAnim,int loadResid,Interpolator interpolator){
		
		return image(url, memCache, fileCache, targetWidth, fallbackId, null, 0, 0, 0, null,loadAnim,loadResid,interpolator);
	}
	...
	public T image(String url, boolean memCache, boolean fileCache, int targetWidth, int fallbackId, Bitmap preset, int animId, float ratio){
		return image(url, memCache, fileCache, targetWidth, fallbackId, preset, animId, ratio, 0, null,0,0,null);
	}
	
	public T image(String url, boolean memCache, boolean fileCache, int targetWidth, int fallbackId, Bitmap preset, int animId, 
			float ratio,int loadAnim,int loadResid,Interpolator interpolator){
		return image(url, memCache, fileCache, targetWidth, fallbackId, preset, animId, ratio, 0, null,loadAnim,loadResid,interpolator);
	}
	
	protected T image(String url, boolean memCache, boolean fileCache, int targetWidth, int fallbackId, 
			Bitmap preset, int animId, float ratio, int round, String networkUrl,int loadAnim,int loadResid,Interpolator interpolator){
		...
		if(view instanceof ImageView){		
			BitmapAjaxCallback.async(act, getContext(), (ImageView) view, url, memCache, fileCache, targetWidth, fallbackId, preset, animId, ratio, AQuery.ANCHOR_DYNAMIC, progress, ah, 
					policy, round, proxy, networkUrl,loadAnim,loadResid,interpolator);			
			reset();
		}
	 ...
	 *�����ļ�����Ajax�����ͼ�����URLû�л��淵��null��
	 * @param url 
	 * @return File
	 */
	public File getCachedFile(String url){
		return getCachedFile( null,url);
	}
	public File getCachedFile(File file,String url){
		File cacheFile  =( file==null? AQUtility.getCacheDir(getContext(), AQuery.CACHE_PERSISTENT):file);
		File result = AQUtility.getExistedCacheByUrl(cacheFile, url);
	...
	public Bitmap getCachedImage(String url, int targetWidth){
		...
		if(result == null){
			File file_image = BitmapAjaxCallback.changeFilePath(AQUtility.getCacheDir(getContext(),  AQuery.CACHE_DEFAULT));
			File file = getCachedFile(file_image,url);
